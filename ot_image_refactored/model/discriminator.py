...
class ResNet_D(nn.Module):
    def __init__(self, size=64, nc=3, nfilter=64,
        ...
        self.conv_img = spectral_norm(nn.Conv2d(nc, 1*nf, 3, padding=1))
        if use_silu:
        ...
        self.fc = spectral_norm(nn.Linear(self.nf0*s0*s0, 1))
...
class ResNetBlock(nn.Module):
    def __init__(self, fin, fout, fhidden=None,
        ...
        # Submodules
        self.conv_0 = spectral_norm(nn.Conv2d(self.fin, self.fhidden, 3, stride=1, padding=1, bias=self.is_bias))
        if self.bn:
            self.bn2d_0 = nn.BatchNorm2d(self.fhidden)
        self.conv_1 = spectral_norm(nn.Conv2d(self.fhidden, self.fout, 3, stride=1, padding=1, bias=self.is_bias))
        if self.bn:
